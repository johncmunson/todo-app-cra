...
export const addTodo = async (name: string) => {
  const { data: { result: todo } } = await axios.post<JsonRPC<RawTodo>>(baseUrl, {
    jsonrpc: '2.0',
    method: 'addTodo',
    id: 1,
    params: {
...
export const fetchTodos = async () => {
  const { data: { result: todos } } = await axios.post<JsonRPC<RawTodo[]>>(baseUrl, {
    jsonrpc: "2.0",
    method: "getTodoList",
    id: 1
  })
...
export const updateTodo = async (id: string, changes: Partial<Todo>) => {
  ...
  const { data: { result: updatedTodo } } = await axios.post<JsonRPC<RawTodo>>(baseUrl, {
    jsonrpc: '2.0',
    method: 'updateTodo',
    id: 1,
    params: rawTodo
